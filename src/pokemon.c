...
void CreateMonSp(struct Pokemon *mon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId, u16 fixedHp, u16 fixedAbility)
{
    ZeroMonData(mon);
    CreateBoxMon(&mon->box, species, level, fixedIV, hasFixedPersonality, fixedPersonality, otIdType, fixedOtId);

    if(fixedAbility != 0){
        SetMonData(mon, MON_DATA_ABILITY_NUM, &fixedAbility);
    }
    ...
    SetMonData(mon, MON_DATA_MAX_HP, &fixedHp);
    SetMonData(mon, MON_DATA_HP, &fixedHp);
}
...
u16 GetAbilityBySpecies(u16 species, u16 abilityNum)
{
    return gAbilitiesInfo[abilityNum].ability;
}

u16 GetMonAbility(struct Pokemon *mon)
{
    return gAbilitiesInfo[GetMonData(mon,MON_DATA_ABILITY_NUM,NULL)].ability;
}
...
void PokemonToBattleMon(struct Pokemon *src, struct BattlePokemon *dst)
{
    ...
    dst->ability = gAbilitiesInfo[dst->abilityNum].ability;
