...
// my own
void CreateScriptedWildMonSp(u16 species, u8 level, u16 item, u16 hp, u16 ability)
{
    u8 heldItem[2];

    ZeroEnemyPartyMons();
        CreateMonSp(&gEnemyParty[0], species, level, USE_RANDOM_IVS, 0, 0, OT_ID_PRESET, 0, hp, ability);
    if (item)
    {
        heldItem[0] = item;
        heldItem[1] = item >> 8;
