...
} static const sSaveSlotLayout[NUM_SECTORS_PER_SLOT] =
{
    ...
    SAVEBLOCK_CHUNK(struct SaveBlock1,  9), // SECTOR_ID_SAVEBLOCK1_END
    SAVEBLOCK_CHUNK(struct SaveBlock1,  10), // SECTOR_ID_SAVEBLOCK1_END
...
static u8 GetSaveValidStatus(const struct SaveSectorLocation *locations)
{
    ...
    if (signatureValid)
    {
        if (validSectorFlags == (1 << NUM_SECTORS_PER_SLOT) - 1)
            saveSlotStatus = SAVE_STATUS_OK;
        else
            saveSlotStatus = SAVE_STATUS_ERROR;
    }
    else
    {
        // No sectors in slot 1 have the correct signature, treat it as empty
        saveSlotStatus = SAVE_STATUS_EMPTY;
    }

    return saveSlotStatus;
}
